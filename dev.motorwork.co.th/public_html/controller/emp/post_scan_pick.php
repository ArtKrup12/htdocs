...
//    UPDATE `tb_assign` SET `assign_status`='[value-10]',`modify_dt`='[value-11]' WHERE assign_no = '' AND so_id = '' AND assign_task = 'pick' ANS assign_status = 11
    $update_assign = "UPDATE `tb_assign` SET `assign_status`=31,`modify_dt`='$currentDateTime' WHERE assign_no = '$assign_no' AND so_id = '$so_no' AND assign_task = 'pick' AND assign_status = 21";
    mysqli_query($mysqli, $update_assign);

    $update_sale_order = "UPDATE `tb_sale_order` SET `modify_dt`='$currentDateTime',`pick_status`=31 WHERE so_id = '$so_no'";
...
if ($action == 'save_list_so_detail') {
    ...
    if ($sub_action == 'Y') {
        ...
        $sql_insert_pick = "INSERT INTO `tb_pick`(`assign_no`, `so_detail_no`, `assign_round`, `pick_qty`, `flag_pick`, `create_dt`) 
        ...
        $sql_update_so_detail = "UPDATE `tb_sale_order_detail` SET `modify_dt`='$currentDateTime',`flag_pick`='Y' WHERE so_id = '" . $so_no . "' AND prod_location = '" . $prod_location . "'
                                            AND `prod_id` = '" . $prod_id . "'";
    ...
    if ($sub_action == 'N') {
        ...
        $sql_insert_remark_pick = "INSERT INTO `tb_pick_remark`(`pick_no`, `emp_id`,`emp_type`, `remark_id`, `remark`, `create_dt`) 
                        VALUES ('$pick_no','" . $so_data->delegatee_id . "',1,'$remark_type','$remark_text','$currentDateTime')";
        $mysqli->query($sql_insert_remark_pick);

        $sql_update_so_detail = "UPDATE `tb_sale_order_detail` SET `modify_dt`='$currentDateTime',`flag_pick`='N' WHERE so_id = '" . $so_no . "' AND prod_location = '" . $prod_location . "'
                                            AND `prod_id` = '" . $prod_id . "'";
        $mysqli->query($sql_update_so_detail);
...
if ($action == 'save_so') {
    ...
    if ($sub_action == 'N') {
        ...
        $stop_dt = $date1Obj->format('Y-m-d H:i:s');
         ...
         $sql_update_as = "UPDATE `tb_assign` SET `assign_status`='51',`modify_dt`='$currentDateTime' WHERE  so_id = '" . $so_no . "' AND assign_no = '" . $assign_no . "'
                                            AND `assign_task` = 'pick'";
        ...
        $sql_update_so = "UPDATE `tb_sale_order` SET `modify_dt`='$currentDateTime',`pick_status`='51' WHERE so_id = '$so_no'";
    ...
    if ($sub_action == 'Y') {
        ...
        $sql_update_as = "UPDATE `tb_assign` SET `assign_status`='41',`modify_dt`='$currentDateTime' WHERE  so_id = '" . $so_no . "' AND assign_no = '" . $assign_no . "'
                                            AND `assign_task` = 'pick'";
        $mysqli->query($sql_update_as);
        ...
        $sql_update_so = "UPDATE `tb_sale_order` SET `modify_dt`='$currentDateTime',`pick_status`='41' WHERE so_id = '$so_no'";
        $mysqli->query($sql_update_so);
